	...
	while (msg_nb) {
		...
		if (ctx->peer == NULL && ctx->cfg->mode == SC_MODE_DPU) {
			if (sigwait(&signal_mask, &signal) != 0) {
				...
				// 		DOCA_LOG_INFO("SEND START TIME: %ld.%09ld seconds", 
              	// 		start_time.tv_sec, start_time.tv_nsec);
				// 		DOCA_LOG_INFO("Starting message transmission. Will send %d messages of size %d bytes", 
				// 			ctx->cfg->send_msg_nb, ctx->cfg->send_msg_size);
				// 		print_once = 0;
				// 	}
				// }
		...
		if (msg_nb==0)
		{
			...
			DOCA_LOG_INFO("SEND END TIME: %ld.%09ld seconds", 
...
static void *recvfrom_channel(void *context)
{
	...
	while (1) {
		...
		if (result == DOCA_SUCCESS )
		{
			...
			{
				clock_gettime(CLOCK_MONOTONIC, &start_time);
				DOCA_LOG_INFO("Connection has been established");
				DOCA_LOG_INFO("RECV START TIME: %ld.%09ld seconds", 
              	start_time.tv_sec, start_time.tv_nsec);
				print_once = 0;
			}
			...
			{
				clock_gettime(CLOCK_MONOTONIC, &end_time);
				DOCA_LOG_INFO("RECV END TIME: %ld.%09ld seconds", 
              end_time.tv_sec, end_time.tv_nsec);
			}

		}
		pthread_res = pthread_mutex_unlock(ctx->mutex);
		//这里按助教pdf修改的,已经显示了传输进度
		//但这个写的不太对，没收到消息的时候也打印，导致打印空值，所以我改掉了
		//DOCA_LOG_INFO("Received message is: %s", recv_buffer);
		memset(recv_buffer, 0, sizeof(recv_buffer));
		if (pthread_res != 0) {
			DOCA_LOG_ERR("Failed to lock Comm Channel for reading");
			    ...
			    struct doca_dev_rep **dev_rep)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to open Comm Channel DOCA device based on PCI address: %s", doca_error_get_descr(result));
	...
	if (cfg->mode == SC_MODE_DPU) {
		result = open_doca_device_rep_with_pci(*dev,
						       ...
						       dev_rep);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to open Comm Channel DOCA device representor based on PCI address: %s", doca_error_get_descr(result));
			doca_comm_channel_ep_destroy(ctx->ep);
			doca_dev_close(*dev);
			return result;
		}
...
doca_error_t sc_start(struct sc_config *cfg, struct cc_ctx *ctx)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to initialize thread synchronization: %s", doca_error_get_descr(result));
		return result;
	}
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to initialize Comm Channel: %s", doca_error_get_descr(result));
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to initialize signaling polling: %s", doca_error_get_descr(result));
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to start threads: %s", doca_error_get_descr(result));
		close_fd(cc_send_epoll_fd, cc_recv_epoll_fd, send_intr_fd, recv_intr_fd);
		destroy_cc(ctx, dev, dev_rep);
		pthread_mutex_destroy(&cc_mutex);
		return result;
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Send thread finished unsuccessfully: %s", doca_error_get_descr(result));
	}

	result = ctx->results->recvfrom_result;
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Receive thread finished unsuccessfully: %s", doca_error_get_descr(result));
	}
...
doca_error_t register_secure_channel_params(struct sc_config *app_cfg, const char *json_file)
{
	doca_error_t result;
